...
export const p_reportGraphviz: inf.PReportGraphviz = ($, $i, $d) => {
    ...
    function doCategory(name: string) {
        ...
        $d.sortedForEach($.projects, (project, key) => {
            ...
            $d.sortedForEach(
                ...
                (part, key) => {
                    if (part.isPublic) {
                        const partIsDirty = part.status[0] !== "clean"
                            ? true
                            : part.dependenciesDirty
    ...
    $d.sortedForEach(pw.wrapRawDictionary(buckets), ($, key) => {
        $d.sortedForEach(pw.wrapRawDictionary($), ($, key) => {
            $i.log(`\t{ rank=same`)
            $.getArray().forEach(($) => {
                $i.log(`\t\t"${$}"`)
            })
